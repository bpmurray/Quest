...
' All GOTO statements removed and replaced with structured logic
' ================================================================
...
Dim Shared ItemIndex As Integer
Dim Shared SearchIndex As Integer
Dim Shared ListOffset As Integer
...
Dim Shared DataBuffer As String ' ZZ$ - Data buffer for file operations
...
Dim Shared Command As String      ' FIX: uncommented, was commented out
...
Sub InitializeGame
    ...
    CarryCount = 0
    SecondaryAction = "0"
    MoveCount = 1         ' FIX: was 100, set to 1 to avoid divide-by-zero in score
    PenaltyPoints = 0
    ActionResult = "N"
    OutputBuffer = ""
End Sub
...
Sub LoadArtifacts
    ...
    Do
        Call GetRecord
        Dim idx As Integer
        idx = Val(Mid$(RecordContent, 91, 2))
        ArtifactLocation(idx) = Val(Mid$(RecordContent, 93, 4))
        ArtifactRecord(idx) = CurrentRecord
...
Sub LoadGremlins
    ...
    Do
        ...
        Dim idx As Integer
        idx = Val(Mid$(RecordContent, 91, 2))
        GremlinLocation(idx) = Val(Mid$(RecordContent, 93, 4))
        GremlinRecord(idx) = CurrentRecord
        GremlinFactor(idx) = Val(Mid$(RecordContent, 81, 1))
        CurrentRecord = Val(Left$(RecordContent, 4))
    Loop While CurrentRecord > 0
End Sub
...
Sub ArriveAtLocation
    ...
    ' NOTE: Original code used RecentPlaces(Index - 1) causing index 0 on first loop.
    ' Keeping original behavior for compatibility, but preventing index 0 access.
    Dim tmpRecent(5) As Integer
    For Index = 1 To 5
        tmpRecent(Index) = RecentPlaces(Index)
    Next Index
    For Index = 2 To 5
        RecentPlaces(Index - 1) = tmpRecent(Index)
    Next Index
...
Sub GetRecord
    ...
    ' FIX: clarified indexing. Original added +1, skipping record 0.
    ' Assuming QDATA.dat is 1-based, keep +1; guard lower bound.
    RecordNumber = CurrentRecord + 1
    If RecordNumber < 1 Then RecordNumber = 1  ' FIX: guard against <1
...
Sub ExtractWord(ByRef InputString As String)
    ' GOTO REMOVED: trim loop and index math
...
Sub ProcessTextOutput
    ' GOTO REMOVED: reworked into a linear loop over segments
    ' Handle text formatting and output
    If Len(TextOutput) = 0 Then Exit Sub
    
    ' Ensure trailing space after sentences
    ...
    Do While Len(Remaining) > 0
        ...
        Else
            ...
            Select Case Val(codeChar)
                ...
                Case 3: If Len(OutputBuffer) > 0 Then OutputBuffer = Left$(OutputBuffer, Len(OutputBuffer) - 1)
...
Sub HandleDebugCommand
    ...
    ElseIf Word2 = "GREM" Then
        ' Remove all gremlins
        For GremlinIndex = 1 To NumGremlins
            GremlinLocation(GremlinIndex) = -1  ' FIX: standardize gremlin removal
        Next GremlinIndex
    Else
    ...
    For GremlinIndex = 1 To NumGremlins
        ...
        If GremlinLocation(GremlinIndex) = CurrentRoom Then
            ...
            If Rnd * 10 <= GremlinFactor(GremlinIndex) Then
                localTarget = GremlinIndex
                GremlinFactor(GremlinIndex) = 9  ' Reset to high value
            End If
        End If
    Next GremlinIndex
    
    If localTarget = 0 Then Exit Sub
    
    ' Process the acting gremlin
    Call ProcessGremlinAction(localTarget)
End Sub
...
Sub ProcessObjectInteraction
    ...
    Dim descRec As Integer
    descRec = Val(Mid$(RecordContent, 5, 4))
    If descRec <> 0 Then
        If descRec = OptimalMove Then
            TextOutput = "%4and a"
            Call ProcessTextOutput
        Else
            CurrentRecord = descRec
            Call DisplayMessage
        End If
        OptimalMove = descRec
    End If
...
Sub CheckArtifactCommands
    ' GOTO REMOVED: use local found flag instead of NextArtifact
...
Sub HandleCreatureInteraction
    ...
    Dim found As Integer
    found = 0
    Do While localTarget <= NumGremlins And found = 0
        If GremlinLocation(localTarget) = CurrentRoom Then
            CurrentRecord = GremlinRecord(localTarget)
            Call GetRecord
            If Word2 = Mid$(RecordContent, 26, 4) Or Word2 = Mid$(RecordContent, 30, 4) Then
                found = 1
            End If
        End If
        If found = 0 Then localTarget = localTarget + 1
    Loop
    ...
    Command = Left$(Word1, 1) + Mid$(Str$(localTarget * 4 / 100) + "0", 3, 2)
    GremlinData = RecordContent
    TargetGremlin = localTarget
    Call ExecuteAction
End Sub
...
Sub ProcessMandatoryItems
    ...
    Do While L < 10
        ...
        ItemNumber = Mid$(RecordContent, L + 20, 2)
        ...
        ItemNumber = Mid$(RecordContent, L + 32, 2)
        Call ParseItemNumber(ItemNumber)
...
Sub FindActionInRecord
    ...
    Do While S < 90
        If Mid$(RecordContent, S, 1) = "E" Then Exit Sub
        
        If Mid$(RecordContent, S, 3) = Command Then
            Action = Mid$(RecordContent, S + 3, 5)
            Exit Sub
        End If
        
        ' Search for matching action in current record
        If Mid$(RecordContent, S + 1, 2) = "??" And Mid$(RecordContent, S, 1) = Left$(Command, 1) Then
...
Sub HandleDropArtifact(ArtifactIndex As Integer)
    ...
    If CarryCount > 0 Then CarryCount = CarryCount - 1  ' FIX: guard not to go < 0
    Call PlaceArtifact(ArtifactIndex)
End Sub
...
Sub HandleUseArtifact(ArtifactIndex As Integer)
    ...
    ' Default "no action" marker for this sub
    Dim localCommand As String
    localCommand = "X"
    ...
    For ActionIndex = 67 To 75 Step 4
        If Word1 = Mid$(RecordContent, ActionIndex, 4) Then
            localCommand = Right$(Str$((ActionIndex - 63) / 4), 1)
            Exit For
        End If
    Next ActionIndex
    
    If localCommand = "X" Then
    ...
    Command = localCommand + Mid$(Str$(ArtifactIndex / 100) + "0", 3, 2)
    Call ExecuteAction
End Sub
...
Sub ExecuteCarryAction
    ' FIX: removed useless self-assignment
    SecondaryAction = "0"
    Call ExecuteAction
End Sub
...
Sub ProcessRandomTarget(OperationType As String)
    ...
    If OperationType = "1" Then
        ...
        Loop Until ArtifactLocation(RandomIndex) = 0 Or ArtifactLocation(RandomIndex) = CurrentRoom
    ...
    Else
        ...
        Loop Until GremlinLocation(RandomIndex) = CurrentRoom
        
        CurrentRecord = GremlinRecord(RandomIndex)
    End If
    
    FoundTarget = RandomIndex
    Call ProcessTargetAction
End Sub
...
Sub DestroyObject
    ...
    Else
        ' Destroy gremlin
        GremlinLocation(FoundTarget) = -1   ' FIX: use -1 for removed/destroyed
    End If
End Sub
...
Sub ProcessFeedKillCommand
    ...
    Do While itemPos < 7 And matched = 0
        Dim RequiredItem As Integer
        RequiredItem = Val(Mid$(RequiredItems, itemPos, 2))
...
' ----------------------------------------------------------------
' MISSING ACTION EXECUTION ROUTINES (with fixes)
' ----------------------------------------------------------------
Sub ExecuteDropAction
    ...
    If Mid$(Action, 3, 1) = "1" Then
        ...
        If ArtifactLocation(FoundTarget) = 0 Then 
            If CarryCount > 0 Then CarryCount = CarryCount - 1  ' FIX: guard
        End If
        Call PlaceArtifact(FoundTarget)
        CurrentRecord = 346
        Call DisplayMessage
...
Sub ValidateGameState
    ...
    For ArtifactIndex = 1 To NumArtifacts
        If ArtifactLocation(ArtifactIndex) < -1 Then
            ArtifactLocation(ArtifactIndex) = CurrentRoom   ' FIX: clamp invalid to here
        End If
    Next ArtifactIndex
    
    For GremlinIndex = 1 To NumGremlins
        If GremlinLocation(GremlinIndex) < -1 Then
            GremlinLocation(GremlinIndex) = -1             ' FIX: clamp invalid to removed
        End If
    Next GremlinIndex
